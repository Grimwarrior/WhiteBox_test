...
namespace WhiteBox
{
    namespace Api
    {
            ...
            const BooleanOperation operation)
        {
            ...
            // For subtraction use the convex half-space clipper (TrenchBroom-style).
            // It is unconditionally reliable for the convex shapes WhiteBox produces and
            // avoids mcut's MC_INVALID_VALUE failures on valid convex input.
            // Union and Intersection continue to use mcut.
            const bool usedConvex = (csgOperation == Csg::BooleanOperation::Subtraction) &&
                                    Csg::ConvexSubtract(meshA, meshB, result);

            if (!usedConvex && !Csg::MeshBoolean(meshA, meshB, csgOperation, result))
            {
                return false;
            }
